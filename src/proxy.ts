...
function applyCors(res: NextResponse, origin: string | null) {
  ...
  res.headers.set(
    ...
    'Content-Type, Authorization, Idempotency-Key, X-Appointment-Access-Token, X-Kiosk-Token, X-Field-Signature, X-Twilio-Signature, Stripe-Signature'
  );
...
function applySecurityHeaders(response: NextResponse) {
  ...
  response.headers.set(
    ...
    process.env.CSP_HEADER ||
      "default-src 'self'; script-src 'self' 'unsafe-inline' https://js.stripe.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: blob: https:; connect-src 'self' https://api.stripe.com wss: ws:; frame-src https://js.stripe.com; frame-ancestors 'none'"
  );
}
