...
// Gap-ai endpoint: AI consent / form-capture (only present in beauty-wellness-ai)
  ...
  try {
    ...
    const r = await fetch(OPENROUTER_URL, {
      ...
      body: JSON.stringify({ model: OPENROUTER_MODEL, messages: [
        ...
        { role: 'user', content: `Feature: AI consent / form-capture (only present in beauty-wellness-ai)\nContext: ${JSON.stringify(body)}\nReturn JSON {"summary":"","key_points":[""],"recommendations":[""]}` }
    ...
    await persist('ai-consent-form-capture-only-present-in-beauty-wellness-ai', body, out);
    return NextResponse.json({ feature: 'ai-consent-form-capture-only-present-in-beauty-wellness-ai', title: 'AI consent / form-capture (only present in beauty-wellness-ai)', result: out });
  } catch (err: any) {
